...
export const mergeSort: NumsTopic<BarsState> = {
  ...
  codeAlt: {
    ...
    python: {
      ...
      map: [1, 2, 4, 5, 6, 7, 0, 9, 10, 11, 12, 13, 17, 19, 21, 22, 0],
    },
  },
  mistakes: [
    "Using mid = (lo + hi) / 2 with plain division — in Java this is fine for ints, but the same formula overflows for huge arrays; the safe form is lo + (hi − lo) / 2.",
    "Splitting into [lo..mid] and [mid..hi] (overlapping) instead of [lo..mid] and [mid+1..hi] — infinite recursion.",
    "Merging with a[i] < a[j] instead of a[i] <= a[j], which silently breaks stability for equal elements.",
  ],
  interview: [
    "\"Merge k sorted lists\" — the merge step generalized with a heap.",
    "\"Count inversions in an array\" — merge sort with a counter in the merge.",
    "\"Sort a linked list in O(n log n)\" — merge sort is the standard answer (no random access needed).",
  ],
