...
export const dijkstra: TextTopic<GraphState> = {
  ...
  codeAlt: {
    javascript: [
      ...
      "  pq.push([0, start]);",
      "  while (pq.size() > 0) {",
      "    const [d, u] = pq.pop();          // smallest dist",
  ...
  mistakes: [
    "Running it on graphs with negative edges — the greedy finalization is simply wrong there (use Bellman-Ford).",
    "Skipping the stale-entry check after polling, re-relaxing from outdated distances.",
    "Using a plain queue instead of a priority queue — that's BFS, and it only works unweighted.",
  ],
  interview: [
    "\"Network delay time\" — Dijkstra verbatim.",
    "\"Cheapest flights within k stops\" — and why plain Dijkstra needs modification for the stop limit.",
    "\"Path with minimum effort\" — Dijkstra where the cost is the max edge, not the sum.",
  ],
  legend: [
