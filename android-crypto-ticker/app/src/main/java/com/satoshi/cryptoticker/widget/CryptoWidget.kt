...
import com.satoshi.cryptoticker.MainActivity
...
class CryptoWidget : GlanceAppWidget() {
    ...
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        val prices = withContext(Dispatchers.IO) {
            runCatching {
                // Access Hilt-managed API via EntryPoint
                val entryPoint = EntryPointAccessors.fromApplication(
                    context.applicationContext,
                    WidgetEntryPoint::class.java
                )
                entryPoint.coinGeckoApi().getMarkets(ids = "bitcoin,ethereum", sparkline = false)
            }.getOrNull()
        }

        provideContent {
            WidgetContent(prices?.associate { it.id to (it.currentPrice to (it.priceChangePercentage24h ?: 0.0)) } ?: emptyMap())
        }
    }
}
...
private fun WidgetContent(prices: Map<String, Pair<Double, Double>>) {
    Column(
        modifier = GlanceModifier.fillMaxSize().padding(12.dp).appWidgetBackground().background(Color(0xFF161B22)),
